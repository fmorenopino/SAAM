...
import logging
from SAAM_general.SA_module import SA_module
...
class Net(nn.Module):
    ...
    def forward(self, x, hidden, cell, PSD, H):
        '''
    ...
    def getEmbedding(self, PSD_batch):
        ...
        for t in range(self.params.train_window):
            output, (hidden, cell) = self.lstm(PSD_batch[t].unsqueeze_(0).clone(), (hidden, cell))
            hidden_permute = hidden.permute(1, 2, 0).contiguous().view(hidden.shape[1], -1)
    ...
    def get_Rx(self, x):
        """
        On this version I obtain the Rx throught the DFT by: R_x = IFFT(F*F_conj), being F = RFFT(x).
        ...
        #S = torch.from_numpy(np.conj(A.data.cpu().numpy())).to(self.params.device) * A / size
    ...
    def test(self, x, hidden, cell, PSD, sampling=False):
            ...
            for j in range(self.params.sample_times):
                ...
                for t in range(self.params.predict_steps):
                    mu_de, sigma_de, decoder_hidden, decoder_cell, H, _, _,  _, _  = self(x[self.params.predict_start + t].unsqueeze(0),
                                                                        decoder_hidden, decoder_cell, PSD, H)
