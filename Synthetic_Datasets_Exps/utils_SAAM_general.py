...
def save_checkpoint(state, is_best, epoch, checkpoint, ins_name=-1):
    '''Saves DeepAR and training parameters at checkpoint + 'last.pth.tar'. If is_best==True, also saves
    checkpoint + 'best.pth.tar'
    Args:
        state: (dict) contains DeepAR's state_dict, may contain other keys such as epoch, optimizer state_dict
        is_best: (bool) True if it is the best DeepAR seen till now
...
def load_checkpoint(checkpoint, model, optimizer=None):
    '''Loads DeepAR parameters (state_dict) from file_path. If optimizer is provided, loads state_dict of
    optimizer assuming it is present in checkpoint.
    Args:
        checkpoint: (string) filename which needs to be loaded
        model: (torch.nn.Module) DeepAR for which the parameters are loaded
...
def plot_all_epoch(variable, save_name, location='./figures/'):
    ...
    f.savefig(os.path.join(location, save_name + '_summary.pdf'))
    plt.close()
