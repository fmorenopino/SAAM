...
logger = logging.getLogger('SAAM.Utils')
...
def set_logger(log_path):
    ...
    '''
    _logger = logging.getLogger('SAAM')
...
def save_checkpoint(state, is_best, epoch, checkpoint, ins_name=-1):
    '''Saves SAAM and training parameters at checkpoint + 'last.pth.tar'. If is_best==True, also saves
...
def load_checkpoint(checkpoint, model, optimizer=None):
    '''Loads SAAM parameters (state_dict) from file_path. If optimizer is provided, loads state_dict of
...
#Métricas sacadas de ConvTrans
def ND_Metrics(predictions,test_loader, params):
    summation = 0
    diff = 0
    for idx, data in enumerate(test_loader):
        #_,gt,_, _ = data_  # (batch_size,seq, value) --> (seq,batch_size,value)
        _, _, _, gt = data  # (batch_size,seq, value) --> (seq,batch_size,value)
        #gt = gt.permute(1,0).to(torch.float32).to(params.device)
        gt = gt[:, params.predict_start:].permute(1,0).to(torch.float32).to(params.device)
...
def RMSE_Metrics(predictions,test_loader, params):
    summation = 0
    sqr_diff = 0
    count_iterms = 0
    for idx, data in enumerate(test_loader):
...
def Rou_Risk(rou,predictions,test_loader,pred_samples, params):
