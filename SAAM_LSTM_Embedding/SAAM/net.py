...
import logging
from SAAM.SA_module import SA_module

logger = logging.getLogger('SAAM.Net')
...
class Net(nn.Module):
    def __init__(self, params):
        ...
        self.T_f = params.filtering_window
        self.FFT_dim = int(params.NFFT / 2) + 1

        self.lstm_dim = params.lstm_hidden_dim * params.lstm_layers
        self.attn = SA_module(self.lstm_dim, self.T_f, self.FFT_dim).to(params.device)
    ...
    def forward(self, x, idx, hidden, cell, PSD, H):
        '''
        ...
        H = torch.cat((H, hidden_permute.unsqueeze(0)), dim=0)
        h_t_filtered, H_filtered, alpha, FFT_l, attentive_FFT_l = self.attn(hidden_permute, PSD, H)

        pre_sigma = self.distribution_presigma(h_t_filtered)
        mu = self.distribution_mu(h_t_filtered)
        sigma = self.distribution_sigma(pre_sigma)  # softplus to make sure standard deviation is positive

        return torch.squeeze(mu), torch.squeeze(sigma), hidden, cell, H, H_filtered, alpha, FFT_l, attentive_FFT_l

    def getEmbedding(self, PSD_batch, idx):

        onehot_embed = self.embedding(idx)
        onehot_embed = onehot_embed.repeat(self.params.train_window, 1, 1)
        lstm_input = torch.cat((PSD_batch, onehot_embed), dim=2)

        batch_size = PSD_batch.shape[1]
        hidden = self.init_hidden(batch_size)
        cell = self.init_cell(batch_size)

        hidden_full_batch = torch.empty((PSD_batch.shape[0], PSD_batch.shape[1], self.params.lstm_hidden_dim * self.params.lstm_layers))

        for t in range(self.params.train_window):
            output, (hidden, cell) = self.lstm(lstm_input[t].unsqueeze_(0).clone(), (hidden, cell))
            hidden_permute = hidden.permute(1, 2, 0).contiguous().view(hidden.shape[1], -1)
            hidden_full_batch[t,:,:] = hidden_permute.clone()

        return hidden_full_batch
    ...
    def get_Rx(self, x):
        ...
        A = torch.rfft(x, signal_ndim=2, onesided=False).to(self.params.device)
    ...
    def get_Rx_old(self, x):

        """
        :param x: bach of sequences we want to compute the autocorrelation of. Dimensions must be (B,L,F),
        being:
            -B: batch dimension.
            -L: lenght of the sequences (time).
            -F: number of features of the sequences.
        :return: Expectation of the Autocorrelation(E{Rx}) of dimensions (L), which will be used to
        compute the PSD.
        """

        """
        This version, also in pytorch, is the quicker one. It works over all the sequences of the batch and over
        all the features of those sequences (which becomes more important the bigger the embedding is because,
        otherwise, the for loop that iterates throught the features in the previous one would make the algorithm
        much slower). It uses just one for loop to iterate over the sequences length.
        """
        ...
        #Rx = torch.mean(c, dim=1)

        """
        for i in range(Rx.shape[0]):
            plt.close("all")
            plt.plot(Rx.data_.cpu().numpy()[i, :])
            plt.show()
        """


        E_Rx = torch.mean(Rx, 0)
    ...
    def test(self, x, v_batch, id_batch, hidden, cell, PSD, sampling=False):

        batch_size = x.shape[1]
        H = torch.Tensor().to(self.params.device)
        if sampling:
            ...
            for j in range(self.params.sample_times):
                ...
                for t in range(self.params.predict_steps):
                    mu_de, sigma_de, decoder_hidden, decoder_cell, H, _, _,  _, _  = self(x[self.params.predict_start + t].unsqueeze(0),
                                                                         id_batch, decoder_hidden, decoder_cell, PSD, H)
